...
for %%A in ("C:\Users\romjacket\Documents\ra") do SET EMUL=%%~A&if not exist "%%~A" echo.NO EMULATOR>RJerror.log&EXIT /b
for %%A in ("C:\Users\romjacket\Documents\ra\retroarch.exe") do SET EMUZ=%%~nA&if not exist "%%~A" echo.NO EMULATOR EXE>RJerror.log&EXIT /b
...
[INIVARS]
EMUL="C:\Users\romjacket\Documents\ra"
EMUZ="retroarch.exe"
GAMNAM="[GAMNAM]"
...
for /f "delims=" %%a in ('dir /B /A-D "*.bin" "*.zip" "*.ws"') do (
set ROMF=%%~a
set ROM=%%~na
set ROMX=%%~xa
set ROMFN=%%~nxa
...
CMDLINE="[FRLOC]"
CMDXTN="[CMDLINEGET]"
RUNOPTS=" -L "[EMUPATH]\cores\mednafen_wswan_libretro.dll" "
RUNARGS=""
ROMF="[ROMF]"
ROM="[ROM]"
ROMX="[ROMX]"
...
%LAUNCH% "%EMUL%\%EMUZ%.exe" -L "%EMUL%\cores\mednafen_wswan_libretro.dll" "%GAMDIR%\%ROMF%"
%CCMDY%
%RLOC%popd
