...
for %%A in ("") do SET EMUL=%%~A&if not exist "%%~A" echo.NO EMULATOR>RJerror.log&EXIT /b
for %%A in ("\ERROR") do SET EMUZ=%%~nA&if not exist "%%~A" echo.NO EMULATOR EXE>RJerror.log&EXIT /b
...
%LAUNCH% "ERROR" ERROR
for /f "delims=" %%a in ('dir /B /A-D "*ERROR"') do (
set ROMF=%%~a
set ROM=%%~na
set ROMX=%%~xa
set ROMFN=%%~nxa
...
%LAUNCH% "%EMUL%\%EMUZ%.exe"ERROR[ROMIN]ERROR
%CCMDY%
%RLOC%popd
%LAUNCH% "ERROR" ERROR
%XFG%ERROR
