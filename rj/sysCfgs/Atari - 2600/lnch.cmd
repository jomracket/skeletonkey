...
for %%A in ("C:\Download\Emulators\Stella") do SET EMUL=%%~A&if not exist "%%~A" echo.NO EMULATOR>RJerror.log&EXIT /b
for %%A in ("C:\Download\Emulators\Stella\Stella.exe") do SET EMUZ=%%~nA&if not exist "%%~A" echo.NO EMULATOR EXE>RJerror.log&EXIT /b
for %%A in ("%~dpn0") do SET GAMNAM=%%~A
for %%A in ("%EMUL:~0,2%") do SET EMUDIR=%%~A
for %%A in ("%CD%") do SET GAMDIR=%%~A
%CCMDA%
%XPALT%%JYX%for %%A in ("") do (
...
[INIVARS]
EMUL="C:\Download\Emulators\Stella"
EMUZ="Stella.exe"
ANTIMIC=""
XPADDER=""
PLYRN1=""
...
:CPY
%XFG%copy /y "stella.ini" "%EMUL%"
%XFG%copy /y "Basedir.txt" "%EMUL%"
...
for /f "delims=" %%a in ('dir /B /A-D "*.z26" "*.a26" "*.zip" "*.bin" "*.rom" "*.zip"') do (
set ROMF=%%~a
set ROM=%%~na
set ROMX=%%~xa
set ROMFN=%%~nxa
...
%CCMDY%
%RLOC%popd
%XFG%copy /y "%EMUL%\stella.ini" "%GAMDIR%"
