...
for %%A in ("C:\Emulators\Altirra") do SET EMUL=%%~A&if not exist "%%~A" echo.NO EMULATOR>RJerror.log&EXIT /b
for %%A in ("C:\Emulators\Altirra\Altirra64.exe") do SET EMUZ=%%~nA&if not exist "%%~A" echo.NO EMULATOR EXE>RJerror.log&EXIT /b
...
for /f "delims=" %%a in ('dir /B /A-D "*.bin" "*.rom" "*.car" "*.a52" "*.zip"') do (
set ROMF=%%~a
set ROM=%%~na
set ROMX=%%~xa
set ROMFN=%%~nxa
...
CMDLINE=""
CMDXTN="[CMDLINEGET]"
RUNOPTS=" /f /hardware:5200 /kernel:5200 "
RUNARGS=""
ROMF="[ROMF]"
ROM="[ROM]"
ROMX="[ROMX]"
...
%LAUNCH% "%EMUL%\%EMUZ%.exe" /f /hardware:5200 /kernel:5200 "%GAMDIR%\%ROMF%"
%CCMDY%
%RLOC%popd
%XFG%copy /y "%EMUL%\Altirra.ini" "%GAMDIR%"
