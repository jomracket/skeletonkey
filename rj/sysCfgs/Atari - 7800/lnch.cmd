...
CMDLINE=""
CMDXTN="[CMDLINEGET]"
RUNOPTS=" -Fullscreen 1 -MenuEnabled 0 -Region auto "
RUNARGS=""
ROMF="[ROMF]"
ROM="[ROM]"
ROMX="[ROMX]"
...
%LAUNCH% "%EMUL%\%EMUZ%.exe" -Fullscreen 1 -MenuEnabled 0 -Region auto "%GAMDIR%\%ROMF%"
%CCMDY%
%RLOC%popd
...
%XPALT%)
%XPALT%if "%AMCV%" NEQ "" for %%a in ("%AMCV%") do set AMCV=%%a
%XPALT%%XPSTRT% "%ANTIMIC%" --hidden --profile-controller 1 --profile "%FETYPE%.amgp" %AMCP2% %AMCV%
%XPALT%popd
%CCMDZ%
