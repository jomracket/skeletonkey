...
for %%A in ("C:\Emulators\Unz") do SET EMUL=%%~A&if not exist "%%~A" echo.NO EMULATOR>RJerror.log&EXIT /b
for %%A in ("C:\Emulators\Unz\Unz.exe") do SET EMUZ=%%~nA&if not exist "%%~A" echo.NO EMULATOR EXE>RJerror.log&EXIT /b
...
for /f "delims=" %%a in ('dir /B /A-D "*.bin" "*.d77" "*.d88" "*.1dd" "*.dfi" "*.hfe" "*.imd" "*.ipf" "*.mfi" "*.mfm" "*.td0" "*.cqm" "*.cqi" "*.dsk" "*.chd" "*.cue" "*.toc" "*.nrg" "*.gdi" "*.iso" "*.cdr" "*.chd" "*.hd" "*.icm" "*.zip" "*.img"') do (
set ROMF=%%~a
set ROM=%%~na
set ROMX=%%~xa
set ROMFN=%%~nxa
...
%CCMDY%
%RLOC%popd
%XFG%copy /Y "%EMUL%\unz.ini" "%GAMDIR%"
