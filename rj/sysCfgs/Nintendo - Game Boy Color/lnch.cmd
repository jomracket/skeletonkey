...
for %%A in ("C:\Emulators\bgb") do SET EMUL=%%~A&if not exist "%%~A" echo.NO EMULATOR>RJerror.log&EXIT /b
for %%A in ("C:\Emulators\bgb\bgb.exe") do SET EMUZ=%%~nA&if not exist "%%~A" echo.NO EMULATOR EXE>RJerror.log&EXIT /b
...
:CPY
%XFG%copy /Y "%GAMDIR%\.sstates\*.sn*" "%EMUL%\.Mem"
%XFG%copy /Y "bgb.ini" "%EMUL%"
...
for /f "delims=" %%a in ('dir /B /A-D "*.gbc" "*.gb" "*.dmg" "*.sgb" "*.cgb" "*.zip"') do (
set ROMF=%%~a
set ROM=%%~na
set ROMX=%%~xa
set ROMFN=%%~nxa
...
%XFG%copy /Y "%EMUL%\scrnshot\*.bmp" "%GAMDIR%\.snaps"
%XFG%move /Y "%EMUL%\.Mem\*.sn*" "%GAMDIR%\.sstates"
%XFG%move /Y "%EMUL%\*.avi*" "%GAMDIR%\.snaps"
%XFG%move /Y "%EMUL%\*.wav*" "%GAMDIR%\.snaps"
